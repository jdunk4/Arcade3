...
// Fixed perimeter positions — three turrets 120° apart at radius 24. Chosen
// so that no matter where the player mines / power-ups, at least one turret
// can see enemies in the arena interior.
const TURRET_POSITIONS = [
  { angle: 0,                  r: 24 },
  { angle: (2 * Math.PI) / 3,  r: 24 },
  { angle: (4 * Math.PI) / 3,  r: 24 },
];
...
export function spawnAllTurrets(chapterIdx) {
  ...
  for (let i = 0; i < TURRET_POSITIONS.length; i++) {
    ...
    turrets.push(_buildTurret(i, x, z, tint));
  }
}
