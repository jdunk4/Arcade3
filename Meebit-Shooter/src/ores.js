...
function _formDepotMegaOre() {
  ...
  // Gold octahedron mega-ore on the depot.
  const oreGeo = new THREE.OctahedronGeometry(1.1, 0);
  ...
  // Halo sphere around it.
  const haloGeo = new THREE.SphereGeometry(1.8, 16, 12);
  const haloMat = new THREE.MeshBasicMaterial({
    color: 0xffd93d, transparent: true, opacity: 0.25, depthWrite: false,
  });
  ...
  _depotMegaOre = {
    ore, halo, oreGeo, oreMat, haloGeo, haloMat,
    lifeT: 0,
  };
...
function _tickDepotMegaOre(dt, time) {
  ...
  m.lifeT += dt;
  // Follow depot position (so the ore rides the depot as it drives off
  // at wave end — a nice visual touch).
  const px = depot ? depot.pos.x : m.ore.position.x;
  const pz = depot ? depot.pos.z : m.ore.position.z;
  // Spin + gentle bob + pulse.
  m.ore.rotation.y += dt * 1.8;
  m.ore.rotation.x += dt * 0.9;
  m.ore.position.x = px;
  m.ore.position.z = pz;
  m.ore.position.y = 1.5 + Math.sin(time * 2) * 0.18;
  m.halo.position.x = px;
  m.halo.position.z = pz;
  m.halo.position.y = 1.5;
  const pulse = 0.5 + 0.5 * Math.sin(time * 4);
  if (m.oreMat) m.oreMat.emissiveIntensity = 2.0 + pulse * 0.8;
  if (m.haloMat) m.haloMat.opacity = 0.22 + pulse * 0.12;
}
...
export function updateDepot(dt, player) {
  ...
  //   depot mega-ore forms → wave ends.
  // _updateOrbitOres returns true on the frame the merge completes.
  const time = performance.now() / 1000;
  const merged = _updateOrbitOres(dt, time);
  _tickDepotMegaOre(dt, time);
  ...
  // Wave completes the frame the merge animation finishes.
  if (merged) return true;
  if (_depotMegaOre && _depotMegaOre.lifeT > 0.8) return true;
  return false;
}
