...
//
// See _currentZoneDefs() below — called inside spawnPowerupZones().
//
...
// Keep a reference to the last-built set so getZoneCount() / any other
// helper that wants to know "how many zones" can answer without needing
// LAYOUT to be live.
const ZONE_DEFS_STATIC_COUNT = 5;
...
export function spawnPowerupZones(chapterIdx) {
  ...
  const defs = _currentZoneDefs();
  for (let i = 0; i < defs.length; i++) {
    zones.push(_buildZone(i, defs[i], tint));
  }
}
