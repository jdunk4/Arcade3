...
// The 5 zones in completion order. Each entry has a label used in the
// objective text + a fixed position. We pick spots spread around the
// arena so the player has to move ~10-15 units between each.
//
// Positions are chosen to avoid the three turret spots (0°/120°/240° at
// r=24) and to avoid the arena center (where mining debris used to land).
const ZONE_DEFS = [
  { id: 'POWER',     label: 'RESTORE POWER',          x:  16, z:  -4 },
  { id: 'TURRETS_A', label: 'LOAD TURRETS A',         x: -12, z:  14 },
  { id: 'TURRETS_B', label: 'LOAD TURRETS B',         x: -16, z: -10 },
  { id: 'RADIO',     label: 'ESTABLISH RADIO COMMS',  x:  12, z:  16 },
  { id: 'EMP',       label: 'LAUNCH EMP MISSILE',     x:   0, z: -18 },
];
...
export function getActiveZone() {
  ...
  if (!z) return null;
  return { id: z.def.id, label: z.def.label, pos: z.pos };
}
