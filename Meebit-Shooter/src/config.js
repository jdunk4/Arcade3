...
export const BONUS_WAVE_CONFIG = {
  ...
  // IMPORTANT: spawnStagger MUST be generous. Each VRM spawn triggers a
  // material compile on the GPU; firing them too close together locks the
  // browser for ~seconds ("game freezes"). The existing civilian system uses
  // 100ms for just 8 meebits — for 111 we need at least this much and more.
  //
  // 80ms × 111 = ~9s to fully spawn the herd. The player starts catching
  // the nearest few within the first second anyway, so the stagger is
  // invisible in practice.
  spawnStagger: 80,              // ms between individual spawns
  // Cap concurrent in-flight VRM loads so the browser doesn't try to
  // resolve 111 network fetches + parses at once even if the stagger
  // scheduling puts a lot of them in overlap.
