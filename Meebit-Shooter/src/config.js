...
export const WEAPONS = {
  ...
  //
  //   PHASE A — DRAIN. Holding fire creates green tendrils from the gun
  //     to up to LIFEDRAIN_MAX_BEAMS enemies in a forward cone. Each
  //     beam ticks damage on the enemy AND charges the gun. When the
  //     charge meter (S.lifedrainCharge, 0..1) hits 1.0 the gun is
  //     READY.
  //
  //   PHASE B — RELEASE. Once charged, the next fire press fires a
  //     SWARM of homing green projectiles — one per enemy among the
  //     N closest enemies in front of the player. Each projectile
  //     deals heavy damage on impact. Charge resets to 0; back to
  //     phase A.
  //
  // Auto-equipped at chapter 7 entry. Player can revert to standard
  // weapons via 1-6 keys / scroll wheel if they want.
  lifedrainer: {
    name: 'LIFEDRAINER',
    fireRate: 0.05,             // drain-tick cadence (every 50ms while held)
    damage: 8,                  // per-tick drain damage per enemy beam
    bullets: 0, spread: 0, speed: 0,
    slot: 'raygun',             // re-uses raygun HTML slot for the icon (no new slot needed)
    color: 0x00ff66,            // signature green
    isLifedrainer: true,
    drainRange: 14,             // beams reach this far forward
    drainConeAngle: 0.55,       // half-angle of the drain cone (radians)
    drainMaxBeams: 5,           // simultaneous tendrils to enemies
    chargeRate: 0.45,           // charge gained per second per beam-touched enemy
                                // (5 enemies × 0.45 = 2.25/sec → full charge in ~0.45s
                                //  in dense fights; with 1 enemy → 2.2 sec)
    swarmCount: 50,             // projectiles fired on release
    swarmDamage: 70,            // per-projectile damage (50 × 70 = 3500 burst)
    swarmRange: 28,             // forward range to find target enemies
    swarmConeAngle: 1.0,        // wider cone for target acquisition (~57°)
    swarmSpeed: 26,             // projectile travel speed
    swarmHoming: 7.5,           // homing turn rate (rad/sec)
  },
  pickaxe: {
    name: 'PICKAXE', fireRate: 0.32, damage: 20, bullets: 0, spread: 0, speed: 0,
    slot: 'pickaxe', color: 0xffd93d, isMining: true, reach: 2.4,
  },
  // Grenade — thrown ballistic AoE. Available on every level, not selected
  // as a regular weapon (doesn't go in the 1-6 slot cycle). Pressing G (or
  // controller B) throws one forward. Explodes on landing or after its
  // fuse, dealing AoE damage. Limited charges per level, restocked on wave
  // start. See tryThrowGrenade() in main.js.
  grenade: {
    name: 'GRENADE',
    fireRate: 0.6,             // throw cooldown (seconds between throws)
    damage: 0,                 // all damage is AoE
    bullets: 1, spread: 0,
    speed: 18,                 // initial horizontal throw speed
    arc: 9,                    // initial vertical launch speed
    fuse: 1.6,                 // detonate this long after throw even if airborne
    slot: 'grenade',
    color: 0x88ff55,
    isGrenade: true,
    explosionRadius: 8.5,      // expanded — chunky AoE clears tight enemy clusters
    explosionDamage: 110,      // matches rocket direct+AoE so it feels punchy
    maxCharges: 3,             // 3 per wave
  },
};
  ...
  if (localWave === 1) {
    ...
    if (chapterIdx === 0) {
      return {
        ...
        oresRequired: 4,         // 4 eggs → 4 charges
        eggCount: 4,
        enemies: waveEnemyMix(wave, chapterIdx),
        spawnRate: Math.min(2.0, 0.5 + wave * 0.12),
        blockFallRate: 0,        // no falling blocks during egg wave
        blockCount: 0,           // egg wave manages its own spawns
  ...
  if (localWave === 2) {
    ...
    if (chapterIdx === 0) {
      return {
        type: 'cannon-load',
        enemies: waveEnemyMix(wave, chapterIdx),
        spawnRate: 0.7,           // light ambient — much lower than ch2-7's 1+
        chargesRequired: 4,
...
//
// Bumped SOLAR (idx 2) + TOXIC (idx 3) from 2.0 → 3.5 because chapter
// hazards (minesweeper grid, Pac-Man ghosts) make the visual scene
// already busy — projectile fire on top was overwhelming the player.
const CHAPTER_RANGED_MULT = [
  1.0,  // 0 INFERNO        — unchanged
  2.2,  // 1 CRIMSON        — 2.2× slower ranged cadence
  3.5,  // 2 SOLAR          — 3.5× slower (was 2.0×)
  3.5,  // 3 TOXIC          — 3.5× slower (was 2.0×)
...
// -------- CHAPTER PROJECTILE FIRE RANGE --------
// Per-chapter multiplier on each ranged enemy's `range` field. Used to
// shrink the radius at which enemies will fire projectiles — prevents
// "sniped from offscreen" feel where a wizard at range 18 hits the
// player from beyond the visible camera area. Lower values = enemies
// must come closer before firing.
//
// SOLAR + TOXIC keep their default range when in flat fields, but the
// chapter overlays (minesweeper grid, ghost mob) make it hard to track
// enemies AND projectiles together. Tightening the range to ~60% of
// normal means enemies have to enter visible range before firing.
const CHAPTER_RANGED_RANGE_MULT = [
  1.0,  // 0 INFERNO
  1.0,  // 1 CRIMSON
  0.6,  // 2 SOLAR  — 60% range (was 100%)
  0.6,  // 3 TOXIC  — 60% range
  1.0,  // 4 ARCTIC
  1.0,  // 5 PARADISE
  1.0,  // 6 PARADISE FALLEN
];

export function getChapterRangedRangeMult(chapterIdx) {
  const i = Math.max(0, Math.min(CHAPTER_RANGED_RANGE_MULT.length - 1,
    chapterIdx | 0));
  return CHAPTER_RANGED_RANGE_MULT[i];
}
...
export const ENEMY_TYPES = {
  ...
  // that swarm the player. Visually the mega_brute uses the standard
  // brute mesh but at a larger scale and tinted darker — readable as
  // "uh oh, that one's bigger than usual."
  //
  // The "blinks when shot" feedback is implemented via the existing
  // hitFlash mechanic plus a small splits-on-death special-case in the
  // kill path (main.js applyEnemyDamage / killEnemy).
  mega_brute: {
    speed: 0.9,                  // even slower than regular brute (1.2)
    hp: 350,                     // ~2× regular brute (180) — really tanky
    xp: 12,                      // big xp reward
    score: 3000,
    scale: 1.85,                 // 28% bigger than regular brute (1.45)
    damage: 26,
    name: 'MEGA BRUTE',
    splits: true,                // marker — kill path checks this flag
    splitInto: 'roach',          // type to spawn on death
    splitCount: 20,              // 20 roaches per mega_brute (was 5) —
                                 //   user wanted a much heavier swarm
                                 //   that populates all around the brute
    blinkOnHit: true,            // longer hitFlash for visible "blink"
  },
};
