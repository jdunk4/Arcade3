...
export const BONUS_WAVE_CONFIG = {
  ...
  // Stagger between individual spawns. First-time-play chapter 1 runs without
  // the shader pre-warm (no boss fight has happened before wave 1), so spawns
  // pay their shader-compile cost at spawn time. 120ms smears the compile
  // burst widely enough that each frame stays under 16ms even on mid-range
  // GPUs. Subsequent chapters are pre-warmed during the boss fight so spawn
  // is free and this stagger could be much tighter, but we keep it generous
  // for consistency.
  // 120ms × 111 = ~13s to fully spawn the herd. Player is catching nearest
  // pigs from the first second, so tail-end spawn is invisible in practice.
  spawnStagger: 120,             // ms between individual spawns
  // Only 2 concurrent spawns in flight at once. GPU shader compilation is
  // serialized on the driver side anyway; letting 4+ race just means one
  // frame eats all of them instead of spreading across frames.
