...
import { prewarmTurretMats } from './turrets.js';
import { prewarmPowerupMats } from './powerupZones.js';
import { prewarmWavePropsMats } from './waveProps.js';
import { prewarmWireMats } from './empWires.js';
...
export function prewarmShaders(renderer) {
  ...
  // -----------------------------------------------------------------
  // 7g2. Turret materials per chapter tint. Without this the first
  //      wave-2 of each chapter would shader-compile-stall the moment
  //      the dormant turret set is spawned by prepareChapter().
  // -----------------------------------------------------------------
  for (let c = 0; c < CHAPTERS.length; c++) {
    try {
      prewarmTurretMats(CHAPTERS[c].full.grid1);
    } catch (err) { console.warn('[prewarm] turrets', err); }
  }

  // -----------------------------------------------------------------
  // 7g3. Power-up zone materials per chapter tint. Zones spawn during
  //      prepareChapter and light up once wave 2 starts; this gets the
  //      disk/ring/beam shader variants compiled up front.
  // -----------------------------------------------------------------
  for (let c = 0; c < CHAPTERS.length; c++) {
    try {
      prewarmPowerupMats(CHAPTERS[c].full.grid1);
    } catch (err) { console.warn('[prewarm] powerup zones', err); }
  }

  // -----------------------------------------------------------------
  // 7g4. Central compound materials — silo, powerplant, radio tower.
  //      Stage 3a added buildCentralCompound() which spawns these at
  //      chapter start; without this prewarm the first wave-2 of each
  //      chapter would stall the moment those meshes first render.
  // -----------------------------------------------------------------
  for (let c = 0; c < CHAPTERS.length; c++) {
    try {
      prewarmWavePropsMats(CHAPTERS[c].full.grid1);
    } catch (err) { console.warn('[prewarm] wave props', err); }
  }

  // -----------------------------------------------------------------
  // 7g5. Wire materials — dim/lit line tubes + pulse spheres. Each
  //      chapter gets its own tinted set; without this, the first
  //      POWER-completion stalls the frame it lights wires up.
  // -----------------------------------------------------------------
  for (let c = 0; c < CHAPTERS.length; c++) {
    try {
      prewarmWireMats(CHAPTERS[c].full.grid1);
    } catch (err) { console.warn('[prewarm] wires', err); }
  }
  ...
  // -----------------------------------------------------------------
  // 8b. Real render pass — forces shadow-map depth shaders to compile
  //     for every enemy/boss/chapter combo currently parked at y=HIDE_Y.
  //     renderer.compile() only covers main-pass programs; the shadow
  //     pass uses a different camera (the light's shadow camera) and
  //     its depth-only programs get compiled on first real render.
  //     Without this pass, the first frame of Ch.2/3/4/5/6 entering
  //     gameplay hits a shadow-pass shader-compile hitch the moment a
  //     new chapter's enemies first cast shadows. This renders them
  //     all (still hidden at y=-500, below the ground plane) and pays
  //     the cost here during the already-loading startup sequence.
  // -----------------------------------------------------------------
  try {
    renderer.render(scene, camera);
  } catch (err) {
    console.warn('[prewarm] warmup render', err);
  }
