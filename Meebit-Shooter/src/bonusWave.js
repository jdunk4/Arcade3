...
async function _buildPigPool(chapterIdx, countOverride, renderer, camera, onProgress) {
  ...
  // (feels more natural to players than "chapter 0"). Routed through
  // logBonusWaveLine so it lands under the chapter header alongside the
  // herdVrm manifest + prewarmed lines instead of clumping at the bottom.
  const chapterName = ['INFERNO', 'CRIMSON', 'SOLAR', 'TOXIC', 'ARCTIC', 'PARADISE', 'PARADISE FALLEN'][chapterIdx] || '';
  const chapterLabel = `Chapter ${chapterIdx + 1} · ${chapterName} · ${herdId}`;
  logBonusWaveLine(herdId, chapterLabel, `pool ready: ${pool.length} clones`);
}
