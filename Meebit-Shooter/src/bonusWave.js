...
// -- Pig pool (PSO-warm stash) --
// Pre-cloned, pre-rendered, hidden meshes for the current chapter's herd.
// Built during wave 5 (boss fight) via preparePigPool(). On wave 6 start,
// entries are pulled from the pool, teleported into the arena, and made
// visible — skipping the cold-start PSO-compile freeze.
//
// Each entry: { obj: Object3D, herdId: string, filename: string, mixer: ... }
// Meshes are already in the scene (visible=false, parked at y=-1000) with
// matrixAutoUpdate=false so their presence costs ~nothing per frame.
const pigPool = [];
let _poolChapter = -1;           // which chapter's herd the pool currently holds
...
export function clearBonusWave() {
  for (const h of herd) {
    ...
    if (h.fromPool) {
      ...
      const poolEntry = pigPool.find(e => e.obj === h.obj);
 ...
 * per unique material × 111 pigs = seconds of freeze on Wave 6 start.
 *
 * Fix: create the 111 clones during the boss fight (wave 5), add them to
 * the live scene at y=-1000 (below the floor), render them once via
 * renderer.compile(sceneWithClones, camera), then hide them with
 * `visible = false`. This pays the PSO compile cost NOW, in the middle of
 * combat where a few 1-2ms hitches are invisible. When Wave 6 starts, the
 * clones are pulled from the pool, teleported up to y=0, and made visible
 * — zero compile work, zero freeze.
 *
 * @param {number} chapterIdx    — chapter whose herd to build
 * @param {number} count         — how many pool entries to create (default 111)
 * @param {THREE.WebGLRenderer} renderer
 * @param {THREE.Camera} camera
 */
export async function preparePigPool(chapterIdx, count, renderer, camera) {
  // If we already built the pool for this chapter, no-op.
  if (_poolChapter === chapterIdx && pigPool.length > 0) return;

  // Different chapter? Drop the old pool before building the new one.
  if (_poolChapter !== chapterIdx) {
    clearPigPool();
  }
  ...
  _poolChapter = chapterIdx;

  // Build the pool — one entry per Wave 6 slot, cycling through filenames
  // if the herd has fewer unique VRMs than slots.
  //
  // Do this in batches across animation frames so the cloning + scene adds
  // don't spike a single frame. 3 clones per frame × ~37 frames ≈ 600ms of
  // cloning spread across the boss fight.
  ...
  for (let i = 0; i < count; i++) {
    ...
    mesh.updateMatrix();   // one-time update so world matrix is set
    mesh.traverse(o => {
      o.frustumCulled = false;
    });
    scene.add(mesh);
    tmpScene.add(mesh);    // also add to temp scene for the compile pass
  ...
  // PSO-warm all the clones against the current camera at once. This is
  // what actually compiles the driver pipeline state so the first-draw
  // freeze is paid here instead of on Wave 6 spawn.
  //
  // IMPORTANT: after compile, return the clones to `scene` (they were
  // temporarily parented to tmpScene for the compile pass). Three.js doesn't
  // actually render tmpScene; compile just walks the tree to prime shaders.
  try {
    if (renderer && camera) {
  ...
  console.info(`[bonusWave] pig pool ready: ${pigPool.length} clones for chapter ${chapterIdx} (${herdId})`);
}
...
async function _spawnOne(slotIdx, filename) {
  ...
  // POOL PATH — find a pool entry for the same herd that's not yet in use.
  // If the pool was built for this chapter during the boss fight (wave 5),
  // this is a zero-freeze operation: the mesh is already in the scene with
  // shaders + PSOs compiled, so "spawning" is just toggling visible=true and
  // teleporting. No GLTF parse, no clone, no shader compile.
  let pooled = null;
  if (_poolChapter === currentChapterIdx) {
    for (let i = 0; i < pigPool.length; i++) {
      const e = pigPool[i];
...
function _save(h, idx) {
  ...
  if (h.fromPool) {
    const poolIdx = pigPool.findIndex(e => e.obj === h.obj);
    if (poolIdx >= 0) pigPool.splice(poolIdx, 1);
  }
