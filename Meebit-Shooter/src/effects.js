...
// Per-tint cached goo material. First spawn of each color used to
// trigger a shader compile stall; now cached.
const _gooMatCache = new Map();
function _getGooMat(tintHex) {
  let m = _gooMatCache.get(tintHex);
  if (!m) {
    m = new THREE.MeshBasicMaterial({
      color: tintHex, transparent: true, opacity: 0.75, side: THREE.DoubleSide,
    });
    _gooMatCache.set(tintHex, m);
  }
  return m;
}

export function spawnGooSplat(x, z, tintHex) {
  // Clone so each splat can fade independently. Clone is free — the
  // shader program is reused via programCacheKey.
  const mat = _getGooMat(tintHex).clone();
...
// Per-tint cached materials for boss cubes (rain during boss fight).
const _bossCubeMatCache = new Map();
const _bossRingMatHatch = new THREE.MeshBasicMaterial({
  color: 0xffd93d, transparent: true, opacity: 0.7, side: THREE.DoubleSide,
});
const _bossRingMatExplode = new THREE.MeshBasicMaterial({
  color: 0xff2e4d, transparent: true, opacity: 0.7, side: THREE.DoubleSide,
});
const BOSS_CUBE_RING_GEO = new THREE.RingGeometry(1.2, 1.6, 24);

function _getBossCubeMat(tintHex) {
  let m = _bossCubeMatCache.get(tintHex);
  if (!m) {
    m = new THREE.MeshStandardMaterial({
      color: tintHex, emissive: tintHex, emissiveIntensity: 1.2, roughness: 0.4,
    });
    _bossCubeMatCache.set(tintHex, m);
  }
  return m;
}

export function spawnBossCube(x, z, tintHex, mode /* 'hatch' | 'explode' */) {
  const mat = _getBossCubeMat(tintHex);
  const cube = new THREE.Mesh(BOSS_CUBE_GEO, mat);
  cube.position.set(x, 25, z);
  cube.castShadow = true;
  scene.add(cube);
  // Ground shadow warning indicator — cloned so each cube can animate
  // its ring opacity independently during the fuse flash.
  const baseRingMat = mode === 'explode' ? _bossRingMatExplode : _bossRingMatHatch;
  const ringMat = baseRingMat.clone();
  const ring = new THREE.Mesh(BOSS_CUBE_RING_GEO, ringMat);
  ring.rotation.x = -Math.PI / 2;
...
// ============================================================
// ============================================================
//  CAPTURE ZONE — cached shared materials/geometries
// ============================================================
// Golden color used by all capture-zone parts. Cached so the first
// capture wave (wave 3 of every chapter) doesn't stall.
const _CAP_GOLD_OPAQUE = 0xffd93d;
const CAP_INNER_GEO = new THREE.CircleGeometry(CAPTURE_RADIUS - 0.2, 48);
const CAP_BOUNDARY_GEO = new THREE.RingGeometry(CAPTURE_RADIUS - 0.1, CAPTURE_RADIUS + 0.2, 64);
const CAP_RING_GEO = new THREE.RingGeometry(CAPTURE_RADIUS + 0.3, CAPTURE_RADIUS + 0.6, 48, 1, 0, 0.1);
const CAP_BEAM_GEO = new THREE.CylinderGeometry(0.5, 0.5, 16, 16, 1, true);
const CAP_MISSILE_GEO = new THREE.ConeGeometry(0.8, 2.5, 6);
const CAP_INNER_MAT = new THREE.MeshBasicMaterial({
  color: _CAP_GOLD_OPAQUE, transparent: true, opacity: 0.15, side: THREE.DoubleSide,
});
const CAP_BOUNDARY_MAT = new THREE.MeshBasicMaterial({
  color: _CAP_GOLD_OPAQUE, transparent: true, opacity: 0.8, side: THREE.DoubleSide,
});
const CAP_BEAM_MAT = new THREE.MeshBasicMaterial({
  color: _CAP_GOLD_OPAQUE, transparent: true, opacity: 0.3, side: THREE.DoubleSide,
});
const CAP_MISSILE_MAT = new THREE.MeshStandardMaterial({
  color: _CAP_GOLD_OPAQUE, emissive: _CAP_GOLD_OPAQUE, emissiveIntensity: 0.8,
});
...
export function makeCaptureZone(x, z) {
  ...
  const inner = new THREE.Mesh(CAP_INNER_GEO, CAP_INNER_MAT);
  inner.rotation.x = -Math.PI / 2;
  inner.position.y = 0.04;
  group.add(inner);

  const boundary = new THREE.Mesh(CAP_BOUNDARY_GEO, CAP_BOUNDARY_MAT);
  boundary.rotation.x = -Math.PI / 2;
  boundary.position.y = 0.05;
  group.add(boundary);

  // Progress ring needs a fresh material per zone because it animates
  // its opacity independently — but geometry and shader are shared.
  const ringMat = new THREE.MeshBasicMaterial({
    color: _CAP_GOLD_OPAQUE, transparent: true, opacity: 1.0, side: THREE.DoubleSide,
  });
  const ring = new THREE.Mesh(CAP_RING_GEO, ringMat);
  ring.rotation.x = -Math.PI / 2;
  ring.position.y = 0.06;
  group.add(ring);

  const beam = new THREE.Mesh(CAP_BEAM_GEO, CAP_BEAM_MAT);
  beam.position.y = 8;
  group.add(beam);

  const missile = new THREE.Mesh(CAP_MISSILE_GEO, CAP_MISSILE_MAT);
  missile.position.y = 1.3;
  group.add(missile);

  scene.add(group);
  return {
    obj: group, pos: group.position,
    inner, ring, boundary, beam, missile,
  };
}
