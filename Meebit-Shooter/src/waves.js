...
import { applyTheme, scene } from './scene.js';
...
export function startWave(waveNum) {
  ...
  if (waveDef.type === 'bonus') {
    ...
    const herdInfo = startBonusWave(
      S.chapter,
      chapterForHerd.full.grid1,
      _onBonusCaught,
    );
    UI.toast(
      'BONUS WAVE · ' + (herdInfo.icon || '') + ' ' + herdInfo.label + ' STAMPEDE',
      '#ffd93d', 2800
    );
    UI.showObjective(
      'CATCH THE ' + herdInfo.label,
      'Walk near them to collect · 30 seconds · no damage taken'
    );
    ...
    // Warm the cache for the upcoming bonus wave while the boss is alive.
    // Non-blocking — if fetches fail, the bonus wave's own fallback handles it.
    prefetchNextHerd(S.chapter);
...
function _onBonusCaught(info) {
  ...
  // Persist the herd meebit ID to the rescued-collection so it flows into
  // the player's 20K counter via Save.onChapterComplete at wave end.
  // Herd IDs are scoped per herd (1..111) — we encode them as "herdId:idx"
  // strings so they don't collide with the numeric public-Meebit IDs.
  const tag = info.herdId + ':' + info.herdIdx;
  if (!S.rescuedIds.includes(tag)) {
    S.rescuedIds.push(tag);
    S.rescuedCount++;
  }
  // Toast only on milestones so the screen doesn't spam 111 times.
  const n = S.bonusCaughtThisWave;
  if (n === 1 || n === 10 || n === 25 || n === 50 || n === 75 || n === 100 || n === 111) {
    UI.toast(n + ' ' + (info.herdLabel || 'CAUGHT') + ' COLLECTED!', '#ffd93d', 1600);
  }
}
...
export function updateWaves(dt) {
  ...
  if (waveDef.type === 'bonus') {
    const state = updateBonusWave(dt, player);
    // Live HUD update
    const t = Math.ceil(state.timeLeft);
    UI.showObjective(
      (state.herdLabel || 'HERD') + ' · ' + state.caught + ' / ' + state.total,
      'Time left: ' + t + 's · walk near them to collect',
    );
    if (state.finished) {
      const final = endBonusWave();
      UI.toast(
        'BONUS WAVE COMPLETE · ' + final.caught + ' / ' + final.total + ' CAUGHT',
        '#ffd93d', 3200
      );
