...
export function startWave(waveNum) {
  ...
  if (!_isBonusWave && !S.hyperdriveActive) {
    // Gated by S.hyperdriveActive: during the 8s ATTACK-THE-AI prelude,
    // the arena is fog-hidden and the player can freely move around.
    // Spawning damage tiles they can't see would be deeply uncool.
    // main.js schedules a hazard respawn call at the t=8s fog reveal.
