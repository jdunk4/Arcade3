...
export function startWave(waveNum) {
  ...
  if (waveDef.type === 'bonus') {
    ...
    //                         ends the wave
    //
    // Zones were spawned as dormant props by prepareChapter. startPowerupWave
    // lights up zone 0; updatePowerupZones advances zones as the player
    // holds each one.
  ...
  if (waveDef.type === 'powerup' && S.powerupActive) {
    ...
    if (completedId) {
      ...
      if (completedId === 'POWER') {
        ...
        UI.toast('RADIO COMMS ESTABLISHED — LAUNCH PREP', '#4ff7ff', 1800);
        // Stage 3a stub: auto-fire the EMP 2 seconds after RADIO. Stage 3b
        // replaces this with the shoot-the-generator mechanic.
        S._empStubTimer = 2.0;
      }
    }

    // Stage 3a stub: once RADIO completes, count down to auto-EMP.
    if (S._empStubTimer !== undefined && S._empStubTimer > 0) {
      S._empStubTimer -= dt;
      if (S._empStubTimer <= 0 && !S.powerupEmpFired) {
        S._empStubTimer = undefined;
    ...
    if (!S.powerupEmpFired) {
      ...
      if (az && az.turretIdx >= 0) {
        // Only spin while the player is actively making progress (i.e.,
        // inside the zone) — otherwise the spin keeps going when they
        // step out, which reads as "still working" when it shouldn't.
        const progress = getActiveProgress();
        // We don't have an "inside" boolean directly, but progress only
        // goes UP while inside the zone and DECAYS while outside, so
        // tracking a small delta is sufficient. Easier: just spin any
        // time the turret zone is the active one and progress > 0.
        if (progress > 0.001) setTurretCharging(az.turretIdx, true);
      }
    }

    // HUD update — show current zone label + charge percent.
    if (!S.powerupEmpFired) {
      const az = getActiveZone();
      if (az) {
      ...
      } else if (S._empStubTimer !== undefined && S._empStubTimer > 0) {
        UI.showObjective(
          'LAUNCH PREP · EMP IN ' + Math.ceil(S._empStubTimer) + 's',
          'Stage 3b will add the manual-launch generator mechanic.',
        );
      }
    }
  }
...
function endWave() {
  ...
  // _fireEmp() already called endPowerupWave + clearTurretBullets when
  // the EMP fired. This path covers edge cases (e.g. a boss-wave early
  // exit) where we want to be sure nothing leaks.
  if (S.powerupActive) {
    S.powerupActive = false;
    endPowerupWave();
    clearTurretBullets();
