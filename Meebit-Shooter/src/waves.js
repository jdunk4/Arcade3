...
export function updateWaves(dt) {
  ...
  // CHAPTER 1 REFLOW — wave 2 (cannon-load) STUB tick.
  // Counts down 60 seconds and auto-completes the wave so the player
  // can advance to wave 3 (queen-cleanup) and validate the queen +
  // dome visuals end-to-end. Real charge-delivery flow + cannon
  // auto-fire + dome-pop wiring ships in turn 2D.
  if (waveDef.type === 'cannon-load' && S.cannonLoadActive) {
    S.cannonLoadStubT = (S.cannonLoadStubT || 0) + dt;
    const remaining = Math.max(0, S.cannonLoadStubMax - S.cannonLoadStubT);
    UI.showObjective(
      'DELIVER 4 CHARGES TO THE CANNON',
      'STUB · auto-advancing in ' + Math.ceil(remaining) + 's',
    );
    if (S.cannonLoadStubT >= S.cannonLoadStubMax) {
      S.cannonLoadActive = false;
      UI.toast('CANNON FIRED', '#ffaa00', 1800);
      endWave();
      return;
    }
  }
