...
export function updateWaves(dt) {
  ...
  if (waveDef.type === 'mining' && S.miningActive) {
    ...
    if (isEggWave && S._chargeCubesSpawned && (S.chargesCarried || 0) >= 4) {
      // Reset the deposit tracker so a re-run starts clean.
      S._lastEggDeposited = 0;
      // Tell the wave system we're done. No catapult, no shockwave,
      // just the chapter-1 reflow handoff.
      // Charge cubes are already cleaned up by the cube-pickup logic.
  ...
  if (waveDef.type === 'cannon-load' && S.cannonLoadActive) {
    ...
    // Objective text — phase-aware. Wave 2 starts with player already
    // carrying 4 charges (inherited from wave 1's cube collection),
    // so there's no depot-pickup phase. Either they're walking the
    // charges to the cannon or the barrage is firing.
    if ((S.chargesCarried || 0) > 0) {
      UI.showObjective(
