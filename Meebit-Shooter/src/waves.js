...
import {
  activateTurretsUpTo, deactivateAllTurrets, clearTurretBullets,
} from './turrets.js';
import {
...
export function updateWaves(dt) {
  ...
  //   TURRETS_B → bring turrets 1 + 2 online
  //   EMP       → _fireEmp() — drops hive shields, powers down turrets,
  //               ends the wave
  // POWER and RADIO are story beats with no mechanical side-effects.
  if (waveDef.type === 'powerup' && S.powerupActive) {
    ...
    if (completedId) {
      ...
      if (completedId === 'POWER') {
        ...
        activateTurretsUpTo(1);
        UI.toast('TURRET 1 ONLINE', '#ffd93d', 1600);
      } else if (completedId === 'TURRETS_B') {
        activateTurretsUpTo(3);
        UI.toast('TURRETS 2 + 3 ONLINE', '#ffd93d', 1600);
      } else if (completedId === 'RADIO') {
        UI.toast('RADIO COMMS ESTABLISHED', '#4ff7ff', 1600);
      } else if (completedId === 'EMP') {
