...
export function startWave(waveNum) {
  ...
  if (waveDef.type === 'bonus') {
    ...
    // Also pre-clone 111 hidden meshes for this chapter's herd and PSO-warm
    // them in the live scene. By the time Wave 6 starts, these are already
    // rendered-at-least-once with shadows + fog + lights active, so teleport
    // to arena is a zero-freeze operation. See preparePigPool in bonusWave.js.
    preparePigPool(S.chapter, 111, renderer, camera);
