...
class AudioEngine {
  ...
  // Hyperdrive warp — plays when the player clicks ATTACK THE AI.
  // 7-second cinematic sweep matched to the CSS matrix-warp animation.
  // Layered structure:
  //
  //   BUILD-UP (0.0-1.5s):   Just a deep 40Hz sine drone fading in slowly.
  //                          The player hears "something is happening" but
  //                          the arena is still visually calm.
  //   RAMP (1.5-4.5s):       Drone continues. A slow sawtooth sweep from
  //                          80Hz → 600Hz rides on top, doubled a fifth
  //                          higher. Engine is spooling up.
  //   WARP (4.5-7.0s):       Fast sawtooth sweep 600Hz → 3000Hz, hard
  //                          stacked whoosh-noise layer with filter
  //                          opening wide. This is the reference-image
  //                          "stars become streaks" beat.
  //   PUNCH (7.0s):          Boom transient — noise burst + falling
  //                          saw + square squeal. Lines up with the
  //                          white flash.
  //
  // Total audio energy is front-loaded quietly and back-loaded loudly so
  // the player feels the drama build instead of getting blasted from t=0.
  hyperdrive() {
    if (!this.ctx || this.muted) return;

    // LAYER 1 — sub-bass drone, runs the full 7s. Starts 40Hz, ends 60Hz.
    // Gentle swell — gainStart is low so it builds presence gradually.
    this._beep({ type: 'sine', freqStart: 40, freqEnd: 60, dur: 7.0, gainStart: 0.15 });

    // LAYER 2 — RAMP phase engine sweep. Delayed 1.5s so the build-up is
    // quiet. Long slow ramp from 80Hz → 600Hz over 3s.
    this._beep({ type: 'sawtooth', freqStart: 80, freqEnd: 600, dur: 3.0, gainStart: 0.12, delay: 1.5 });
    // Harmony a fifth above for body.
    this._beep({ type: 'sawtooth', freqStart: 120, freqEnd: 900, dur: 3.0, gainStart: 0.09, delay: 1.55 });

    // LAYER 3 — WARP phase full scream. Delayed 4.5s. Fast ramp from
    // 600Hz → 3kHz over 2.5s. This is the engine at full throttle.
    this._beep({ type: 'sawtooth', freqStart: 600, freqEnd: 3000, dur: 2.5, gainStart: 0.35, delay: 4.5 });
    this._beep({ type: 'sawtooth', freqStart: 900, freqEnd: 4500, dur: 2.5, gainStart: 0.22, delay: 4.55 });

    // LAYER 4 — whoosh-noise bursts, spread across the whole timeline
    // with filters opening wider and wider. Each burst layered on top of
    // the previous to build continuous air-rush without gaps.
    [0.5, 1.8, 3.0, 4.2, 5.2, 6.0].forEach((dly, i) => {
      const cutoff = 300 + i * 700;           // filter opens: 300 → 3800Hz
      const gain = 0.04 + i * 0.04;            // gets louder over time: 0.04 → 0.24
      const dur = 1.2 + i * 0.1;               // slightly overlapping
      setTimeout(() => this._noise(dur, cutoff, gain), dly * 1000);
    });

    // LAYER 5 — PUNCH at t=7.0. Loud boom transient that cues the white
    // flash. Shorter than bigBoom so it doesn't drag into gameplay.
    setTimeout(() => {
      this._noise(0.3, 1200, 0.6);
      this._beep({ type: 'sawtooth', freqStart: 220, freqEnd: 30, dur: 0.35, gainStart: 0.55 });
      this._beep({ type: 'square', freqStart: 3200, freqEnd: 500, dur: 0.1, gainStart: 0.22 });
    }, 7000);
  }
}
