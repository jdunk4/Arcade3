...
// Pad indices we've decided to ignore — headsets, single-button accessibility
// pads, or anything else that enumerated as a gamepad but doesn't have
// enough buttons/axes to actually be one. Populated at connect time, read
// in the poll loop. Cleared on disconnect.
const _ignoredPadIndices = new Set();

// Right-stick aim hold — grace period after the stick returns to neutral,
// during which mouse._gamepadAiming stays true so main.js doesn't snap the
// aim back to auto-aim-to-nearest-enemy between flicks of the stick.
let _aimHoldTimer = 0;
...
export function initGamepad(opts = {}) {
  ...
  window.addEventListener('gamepadconnected', (e) => {
    ...
    // Reject non-controller devices that Windows/macOS enumerate as
    // gamepads. The most common culprits: gaming headsets (e.g. Turtle
    // Beach Stealth 600X) expose mute/volume/voice-chat buttons as a
    // "game controller" HID device. They typically have < 10 buttons
    // and 0-1 axes. A real controller has 16-17 buttons (face + d-pad +
    // shoulders + triggers + stick clicks + home) and 4 axes (two sticks).
    // Filtering by counts is more portable than maintaining an id
    // denylist that'd break with the next headset model.
    if (pad.buttons.length < 10 || pad.axes.length < 2) {
      console.warn(
        '[gamepad] IGNORING "' + pad.id + '" — looks like a headset or non-controller device ' +
        '(buttons=' + pad.buttons.length + ', axes=' + pad.axes.length + ', need ≥10 buttons and ≥2 axes). ' +
        'Its inputs won\'t drive the game.'
      );
      _ignoredPadIndices.add(pad.index);
      return;
    }
  ...
  window.addEventListener('gamepaddisconnected', (e) => {
    ...
    _ignoredPadIndices.delete(e.gamepad.index);
    const pads = navigator.getGamepads ? navigator.getGamepads() : [];
    let anyStill = false;
    for (const p of pads) {
      if (p && p.connected && !_ignoredPadIndices.has(p.index)) { anyStill = true; break; }
    }
    _connected = anyStill;
...
export function updateGamepad(dt) {
  ...
  for (const p of pads) {
    ...
    // Skip devices we've already flagged as non-controllers (headsets,
    // etc). The Set is populated by gamepadconnected, but on some browsers
    // that event doesn't fire for devices already plugged in at page load
    // — so also re-check the same button/axis threshold here every frame.
    // This makes the filter robust regardless of event timing.
    if (_ignoredPadIndices.has(p.index)) continue;
    if (p.buttons.length < 10 || p.axes.length < 2) {
      // First-time sighting of a low-count device — log and remember. On
      // subsequent frames we skip it via the Set above instead of re-logging.
      if (!_ignoredPadIndices.has(p.index)) {
        console.warn(
          '[gamepad] IGNORING "' + p.id + '" — looks like a headset or non-controller device ' +
          '(buttons=' + p.buttons.length + ', axes=' + p.axes.length + ', need ≥10 buttons and ≥2 axes).'
        );
        _ignoredPadIndices.add(p.index);
      }
      continue;
    }
    pad = p;
  ...
  if (!pad) {
    ...
    // Release the right-stick aim lock so mouse / auto-aim works again.
    if (mouse._gamepadAiming) {
      mouse._gamepadAiming = false;
      _aimHoldTimer = 0;
    }
    return;
  }
  ...
  // --- Right stick → aim ---
  // When the right stick is past its deadzone, we drive the aim point
  // directly and flag _gamepadAiming so main.js's auto-aim-to-nearest-
  // enemy logic stays out of the way. When the stick returns to neutral,
  // we clear the flag after a short hold period — this lets the player
  // tap the stick for a quick direction change without the auto-aim
  // immediately snatching their aim back on the next frame.
  ...
  if ((rx !== 0 || ry !== 0) && _player && _player.pos) {
    ...
    _aimHoldTimer = 0.35;  // keep the flag alive this long after release
  } else if (mouse._gamepadAiming) {
    // Stick is neutral. Hold the aim flag briefly so the player can let
    // go and the cursor doesn't instantly teleport to auto-aim.
    _aimHoldTimer -= dt;
    if (_aimHoldTimer <= 0) {
      mouse._gamepadAiming = false;
    }
...
/**
 * Rumble the connected gamepad for `durationMs`. `weak` drives the high-
 * frequency motor (0..1), `strong` drives the low-frequency motor (0..1).
 * No-op if no pad connected, pad doesn't expose vibrationActuator, or
 * vibrationActuator.playEffect isn't implemented by the browser.
 *
 * Used by main.js during the ATTACK THE AI hyperdrive sequence. Safe to
 * call from any context — swallows errors silently so a failed rumble
 * never breaks the game flow.
 */
export function rumble(strong, weak, durationMs) {
  ...
  try {
    ...
    for (const p of pads) {
      if (!p || !p.connected) continue;
      if (_ignoredPadIndices.has(p.index)) continue;
      // Modern Chrome / Edge: pad.vibrationActuator.playEffect
      if (p.vibrationActuator && typeof p.vibrationActuator.playEffect === 'function') {
        ...
        }).catch(() => {});   // swallow unsupported-effect rejections
      }
      // Older Firefox / Edge: pad.hapticActuators[0].pulse(value, duration)
      else if (p.hapticActuators && p.hapticActuators[0] && typeof p.hapticActuators[0].pulse === 'function') {
        // Single-motor fallback — average the two magnitudes.
        p.hapticActuators[0].pulse((strong + weak) * 0.5, durationMs);
      }
    }
  } catch (err) {
    // Never throw — rumble failure shouldn't break game flow.
    console.warn('[gamepad] rumble failed:', err);
  }
}
