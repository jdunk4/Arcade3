...
import { spawnCannon, clearCannon } from './cannon.js';
import { spawnQueenHive, clearQueenHive } from './queenHive.js';
...
export function prepareChapter(chapterIdx) {
  ...
  // The turrets are also skipped — chapter 1 doesn't have the powerup
  // zone flow that activates them.
  if (chapterIdx === 0) {
    // LAYOUT recompute happens inside spawnQueenHive (via getTriangleFor)
    // and spawnCannon (via LAYOUT.silo). LAYOUT was set by buildCentralCompound
    // in past chapters, but we need to call _recomputeLayout via the same
    // entrypoint. Easiest: call buildCentralCompound, then immediately
    // hide the silo/powerplant/radio meshes since they're not used in ch1.
    // (Calling buildCentralCompound also gives us LAYOUT.silo populated
    // for spawnCannon.)
    buildCentralCompound(chapterIdx);
    // Hide the standard compound meshes — they exist but are invisible
    // and untouchable. Cleared on chapter exit by clearCentralCompound.
    ...
    console.info('[dormantProps] prepared chapter 0 (egg/cannon/queen reflow)');
    return;
  }
