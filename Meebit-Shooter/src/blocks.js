...
import { BLOCK_CONFIG, MINING_CONFIG, ARENA, CHAPTERS } from './config.js';
...
export function damageBlock(block, dmg) {
  block.hp -= dmg;
  block.hitFlash = 0.15;
  // Smaller particle burst — this fires a LOT now (up to 100 times per block)
  hitBurst(
    new THREE.Vector3(block.pos.x, block.pos.y + BLOCK_CONFIG.size / 2, block.pos.z),
    block.color, 3
  );
  if (block.hp <= 0) {
    // Break — drop an ORE that the player must carry to the depot.
    spawnOre(block.pos.x, block.pos.z, block.color, block.chapterIdx);

    // Small chance of bonus pickups as a "breaker reward" on top of the ore
    if (Math.random() < 0.35) {
    ...
    shake(0.3, 0.2);   // bigger satisfying shake — the block was a commitment
    return true;
  }
  return false;
}
