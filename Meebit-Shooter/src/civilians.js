...
async function spawnOneCivilian(chapterTintHex) {
  ...
  try {
    ...
    if (mesh.userData.isFallback && mesh.userData.animRefs) {
      ...
      // Real VRM: no named rig refs, but we can still add a walk-cycle bob
      // and a friendly green rim-glow so the player can spot them against
      // the dark arena. The .userData.isFallback check above distinguishes.
