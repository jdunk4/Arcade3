...
function applyCivilianHighlight(mesh, tintHex) {
  ...
  // Stronger than before (was 0.9/4.5) so they really stand out on dark tiles.
  const light = new THREE.PointLight(color, 2.5, 6.0, 2);
  light.position.set(0, 3.4, 0);
  mesh.add(light);

  // Emissive lift — same chapter tint, muted so the base textures still read.
  ...
  mesh.traverse(obj => {
    ...
    for (const m of mats) {
      ...
      // Don't overwrite existing emissive -- some meebits have accent
      // colours baked in that we want to preserve.
