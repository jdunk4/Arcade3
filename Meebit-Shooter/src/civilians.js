...
export function updateCivilians(dt, enemies, player, onCivilianKilled, onCivilianRescued) {
  // ---- DIAG: every 60 frames, log the first 3 civilians' positions.
  // Remove or set _civDiagFrames=0 once positions are animating correctly.
  _civDiagFrames++;
  if (_civDiagFrames % 60 === 0 && civilians.length > 0) {
    const sample = civilians.slice(0, 3).map((c, i) => ({
      i,
      ready: c.ready,
      state: c.state,
      pos: c.pos ? `(${c.pos.x.toFixed(2)}, ${c.pos.y.toFixed(2)}, ${c.pos.z.toFixed(2)})` : 'null',
      objPos: c.obj && c.obj.position ? `(${c.obj.position.x.toFixed(2)}, ${c.obj.position.y.toFixed(2)}, ${c.obj.position.z.toFixed(2)})` : 'null',
      posIsObjPos: c.pos === (c.obj && c.obj.position),
    }));
    console.log('[Civilian DIAG]', sample);
  }
