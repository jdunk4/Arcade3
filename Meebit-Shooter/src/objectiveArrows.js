...
// target type (ORE, HIVE, MEEBIT, etc) and the distance from the
// player.
//
// Target set per wave type:
//   mining        — nearest grounded blocks (up to 5). While the
//                   player is carrying ores, replaced by the depot
//                   arrow so the drop-off becomes the next objective.
//   capture       — the missile-silo capture zone.
//   hive          — every live hive.
//   civilian_rescue — the closest 3 non-following civilians.
//   boss          — the boss (always panic-styled).
//
// VISUAL THEME
//   Arrows are tinted to the CURRENT CHAPTER's signature color (the
//   same tint the crowd + rain + arena pick up). The color is pulled
//   from CHAPTERS[S.chapter].full.grid1 and applied as an inline
//   style so it can update instantly on chapter change.
//
//   Targets flagged `panic` (active-panicking civilians, the boss)
//   always render in a universally-readable red, OVERRIDING the
//   chapter tint — if the player is in immediate danger we don't
//   want the UI camouflaged into the scenery.
//
// DOM strategy: pool of 8 arrow elements created once, reused per
// frame. No allocation. A container div is appended on first use.
...
function _collectTargets(S, waveDef, playerPos) {
  ...
  switch (waveDef.type) {
    ...
    case 'capture': {
      if (S.objectiveZone) {
        _targets.push({
          x: S.objectiveZone.pos.x, z: S.objectiveZone.pos.z,
          label: 'SILO', panic: false,
        });
      }
      break;
    }
    ...
    case 'civilian_rescue': {
      ...
      for (const c of civilians) {
        if (c.dead || c.state === 'following') continue;
        const dx = c.pos.x - px;
        const dz = c.pos.z - pz;
        pool.push({ c, d2: dx * dx + dz * dz });
      }
...
export function updateObjectiveArrows(S, camera, waveDef, playerPos) {
  ...
  for (let i = 0; i < limit; i++) {
    ...
    // On-screen? Hide.
    if (!behind && Math.abs(x) < 0.85 && Math.abs(y) < 0.85) {
    ...
    const sx = (x + 1) * 0.5 * vpW;
    const sy = (-y + 1) * 0.5 * vpH;
    const angle = Math.atan2(-y, x);

    const dx = t.x - (playerPos ? playerPos.x : 0);
    const dz = t.z - (playerPos ? playerPos.z : 0);
