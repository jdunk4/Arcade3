...
async function loadLocalAsset(id) {
  ...
  const promise = (async () => {
    ...
    // VRM / GLB meshes ship with internal armature offsets: the scene root
    // is rarely at (0,0,0) and the visible feet are rarely at y=0 of the
    // root's local space. If we just set scene.position and add to world,
    // the visible geometry drifts away from its logical position -- which
    // is exactly the "civilians are scattered in data but clumped on
    // screen" bug we saw in the diag logs.
    //
    // Fix: wrap the raw scene in a fresh Group, then offset the raw scene
    // inside the wrapper so the wrapper's (0,0,0) aligns with the model's
    // feet at XZ center. Callers then manipulate the wrapper's position
    // normally and the visible geometry follows.
    const wrapper = new THREE.Group();

    // Measure the raw geometry BEFORE any scaling.
    const bbox = new THREE.Box3().setFromObject(rawScene);
    const center = bbox.getCenter(new THREE.Vector3());
    const size = bbox.getSize(new THREE.Vector3());

    // Normalize to a consistent target height (~3.2 world units -- roughly
    // the size of the player avatar and enemies). This also papers over
    // any mixed VRM export scales across the 25 files.
    const TARGET_HEIGHT = 3.2;
    const scale = size.y > 0.001 ? TARGET_HEIGHT / size.y : 1;

    // Re-home the raw scene so:
    //   - feet sit at wrapper's y=0 (bbox.min.y --> 0 after scaling)
    //   - XZ centered on wrapper's origin
